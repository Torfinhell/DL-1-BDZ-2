...
class Attention(nn.Module):
    """
    Excepts tensor of shape (B, L,d_model)->(B, L, d_model)
    relative attention works like attn(i, j)=pos(i, j)+attn(i,j) where 1<=i<=L, 1
    """
    def __init__(self, d_model, num_heads, d_kv, has_realative_attention:bool=False):
        super().__init__()
        self.num_heads = num_heads
        self.has_realative_attention=has_realative_attention
        self.d_kv = d_kv
...
class TransformerConditionalGeneration(nn.Module):
    ...
    def make_causal_mask(self, seq_len, device):
        ...
        return mask.float() * -1e9from torch import nn
import torch
from typing import Optional
import math
